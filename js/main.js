// ═══════════════════════════════════════════════════════════
// MAIN.JS — Luxury Mall · v3 (auto-generado por Admin)
// ═══════════════════════════════════════════════════════════
...
const DEFAULT_INVENTORY = [
  ...
  { id:12, name:"Reloj Cronógrafo Noir", price:890000, stock:2, store:"elegance", type:"joyería",
    ...
    desc:"Acero inoxidable, movimiento suizo, edición limitada.", specs:{"Movimiento":"Suizo automático","Caja":"Acero 316L","Cristal":"Zafiro antirreflejo","Resistencia al agua":"100m"} },
  { id:13, name:"Collar Perlas Barrocas", price:350000, stock:5, store:"elegance", type:"joyería",
    ...
    desc:"Perlas de agua dulce, cierre de oro 18k.", specs:{"Perlas":"Agua dulce barrocas","Cierre":"Oro 18k","Largo":"45 cm","Certificado":"Incluido"} },
  { id:14, name:"Perfume 'Ámbar Nuit'", price:145000, stock:9, store:"elegance", type:"fragancias",
    ...
    desc:"Ámbar, oud y rosa. Fragancia exclusiva 50ml.", specs:{"Familia":"Oriental","Notas base":"Oud, Ámbar","Notas corazón":"Rosa búlgara","Volumen":"50 ml"} },
  { id:15, name:"Aretes Diamante Micro", price:420000, stock:4, store:"elegance", type:"joyería",
    ...
    desc:"0.5ct VVS2, montura de platino.", specs:{"Diamantes":"0.5 ct VVS2","Montura":"Platino 950","Corte":"Brillante","Certificado":"GIA"} },
  ...
  { id:18, name:"Perfume 'Blanche Iris'", price:130000, stock:7, store:"elegance", type:"fragancias",
    ...
    desc:"Iris, musgo blanco y sándalo. 30ml.", specs:{"Familia":"Floral-verde","Notas base":"Sándalo","Notas corazón":"Iris","Volumen":"30 ml"} }
];
...
function getInventory() {
  try {
    ...
    return (parsed && parsed.length) ? parsed : DEFAULT_INVENTORY;
...
function renderGrid(items){
  ...
  grid.innerHTML=items.map(function(item){
    ...
    return '<div class="product-card reveal-card">'+getStockBadge(item.stock)+'<button class="wish-btn '+(inWish?'wishlisted':'')+'" data-wish="'+item.id+'" onclick="cardWish('+item.id+',this)" title="Guardar">♡</button><div class="product-img-wrap"><img src="'+item.img+'" alt="'+item.name+'" loading="lazy"><div class="product-img-overlay" onclick="openModal('+item.id+')"><span>VER DETALLES</span></div></div><span class="product-tag tag-'+item.type+'">'+item.type.toUpperCase()+'</span><div class="product-name">'+item.name+'</div><div class="product-price">₡'+item.price.toLocaleString()+'</div><p class="product-desc">'+item.desc+'</p>'+(hasSpecs?'<button class="add-btn" style="margin-bottom:10px;border-color:rgba(245,240,232,0.1);" onclick="openModal('+item.id+')"><span>VER ESPECIFICACIONES</span></button>':'')+'<button class="add-btn" onclick="addToCart('+item.id+')"><span>AGREGAR A LA BOLSA</span></button></div>';
  }).join('');
