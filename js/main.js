// ═══════════════════════════════════════════════════════════
// MAIN.JS — Luxury Mall · v5 ESTABLE
// ═══════════════════════════════════════════════════════════

const DEFAULT_INVENTORY = [
  // ── DIVINO CIELO — FRAGANCIAS ─────────────────────────────
  ...
  // ── ELEGANCE BEAUTY — SERVICIOS ──────────────────────────
  { id:12, name:"Uñas Acrílicas", price:25000, stock:null, store:"elegance", type:"unas", duration:"60 min",
  ...
  { id:13, name:"Uñas en Gel", price:20000, stock:null, store:"elegance", type:"unas", duration:"60 min",
    img:"https://images.unsplash.com/photo-1604654894610-df63bc536371?q=80&w=800", gallery:[{url:"https://images.unsplash.com/photo-1604654894610-df63bc536371?q=80&w=800",isMain:true}],
    desc:"Acabado natural y brillante de larga duración. Ideal para fortalecer y embellecer la uña natural.", specs:{} },
  { id:14, name:"Diseño de Cejas", price:8000, stock:null, store:"elegance", type:"cejas-pestanas", duration:"30 min",
  ...
  { id:15, name:"Extensión de Pestañas", price:35000, stock:null, store:"elegance", type:"cejas-pestanas", duration:"90 min",
  ...
  { id:18, name:"Limpieza Facial Profunda", price:22000, stock:null, store:"elegance", type:"facial", duration:"60 min",
  ...
  { id:20, name:"Tratamiento Anti-edad", price:40000, stock:null, store:"elegance", type:"facial", duration:"75 min",
  ...
  { id:21, name:"Control de Acné", price:28000, stock:null, store:"elegance", type:"facial", duration:"60 min",
    img:"https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?q=80&w=800", gallery:[{url:"https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?q=80&w=800",isMain:true}],
    desc:"Tratamiento especializado para piel con acné. Limpieza profunda y peelings ácidos.", specs:{} },
  { id:22, name:"Micro Agujas", price:55000, stock:null, store:"elegance", type:"facial", duration:"90 min",
    img:"https://images.unsplash.com/photo-1552693673-1bf958298935?q=80&w=800", gallery:[{url:"https://images.unsplash.com/photo-1552693673-1bf958298935?q=80&w=800",isMain:true}],
    desc:"Microdermoabrasión con agujas para estimular el colágeno. Mejora textura y poros.", specs:{} },
  { id:23, name:"Dermaplaning", price:30000, stock:null, store:"elegance", type:"facial", duration:"45 min",
    img:"https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?q=80&w=800", gallery:[{url:"https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?q=80&w=800",isMain:true}],
    desc:"Exfoliación suave con bisturí para eliminar vello facial fino y células muertas.", specs:{} },
  { id:24, name:"Levantamiento de Glúteos", price:45000, stock:null, store:"elegance", type:"corporal", duration:"60 min",
  ...
  { id:25, name:"Reducción de Grasa", price:50000, stock:null, store:"elegance", type:"corporal", duration:"60 min",
    img:"https://images.unsplash.com/photo-1519415510236-718bdfcd89c8?q=80&w=800", gallery:[{url:"https://images.unsplash.com/photo-1519415510236-718bdfcd89c8?q=80&w=800",isMain:true}],
    desc:"Cavitación ultrasónica para romper células de grasa en zonas específicas.", specs:{} },
  { id:66, name:"Celulitis y Estrías", price:42000, stock:null, store:"elegance", type:"corporal", duration:"60 min",
    img:"https://images.unsplash.com/photo-1519415510236-718bdfcd89c8?q=80&w=800", gallery:[{url:"https://images.unsplash.com/photo-1519415510236-718bdfcd89c8?q=80&w=800",isMain:true}],
    desc:"Tratamiento combinado de presoterapia y radiofrecuencia para mejorar celulitis.", specs:{} }
];

// ── INVENTARIO — fuente de verdad es el DEFAULT hardcodeado ──────────────────
// lm-inventory del browser SOLO se usa si el admin agregó productos nuevos
// (tiene más productos que el DEFAULT). Así NUNCA se puede perder el inventario.
...
function sendWhatsApp(){
  ...
  if (store === 'family') { window.open(waBase + encodeURIComponent(msg), '_blank'); }
  else { window.open(waBase, '_blank'); }
}
...
function getGalleryUrls(item){if(item.gallery&&item.gallery.length>0)return item.gallery.map(function(g){return g.url;});return item.img?[item.img]:[];}
function getMainImg(item){if(item.gallery&&item.gallery.length>0){var m=item.gallery.find(function(g){return g.isMain;});return m?m.url:item.gallery[0].url;}return item.img||'';}
...
function renderModalGallery(urls,activeIdx){
  ...
  var wrap=document.getElementById('modal-gallery-wrap');if(!wrap)return;
  var mainImg=urls[activeIdx];
  var thumbsHTML=urls.length>1?'<div class="modal-thumbs">'+urls.map(function(u,i){return '<div class="modal-thumb '+(i===activeIdx?'active':'')+'" onclick="switchModalImg('+i+')">'+'<img src="'+u+'" alt="foto '+(i+1)+'"></div>';}).join('')+'</div>':'';
  wrap.innerHTML='<div class="modal-img-wrap" id="modal-main-img-wrap" onclick="zoomImg(\''+mainImg+'\')" style="cursor:zoom-in;">'+'<img id="modal-main-img" src="'+mainImg+'" alt="">'+'<div class="modal-zoom-hint">CLICK PARA AMPLIAR</div>'+(urls.length>1?'<button class="modal-gallery-prev" onclick="event.stopPropagation();switchModalImg('+((activeIdx-1+urls.length)%urls.length)+')">‹</button>'+'<button class="modal-gallery-next" onclick="event.stopPropagation();switchModalImg('+((activeIdx+1)%urls.length)+')">›</button>':'')+'</div>'+thumbsHTML;
  if(!document.getElementById('gallery-modal-styles')){var st=document.createElement('style');st.id='gallery-modal-styles';st.textContent='.modal-thumbs{display:flex;gap:6px;margin-top:10px;flex-wrap:wrap;}.modal-thumb{width:56px;height:56px;border:1px solid rgba(245,240,232,0.1);overflow:hidden;cursor:pointer;transition:border-color 0.2s;flex-shrink:0;}.modal-thumb img{width:100%;height:100%;object-fit:contain;}.modal-thumb.active{border-color:var(--gold,#c9a84c);}.modal-gallery-prev,.modal-gallery-next{position:absolute;top:50%;transform:translateY(-50%);background:rgba(8,8,8,0.6);border:1px solid rgba(245,240,232,0.15);color:#fff;width:32px;height:32px;font-size:1.2rem;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background 0.2s;z-index:3;}.modal-gallery-prev{left:6px;}.modal-gallery-next{right:6px;}.modal-gallery-prev:hover,.modal-gallery-next:hover{background:rgba(201,168,76,0.3);}';document.head.appendChild(st);}
}

function switchModalImg(idx){
  _modalGalleryIdx=idx;
  var img=document.getElementById('modal-main-img');
  if(img){img.style.opacity='0';img.style.transition='opacity 0.2s';setTimeout(function(){img.src=_modalGallery[idx];img.style.opacity='1';},180);}
...
function openModal(id){
  ...
  var specsHTML='';
  if(item.specs&&Object.keys(item.specs).length>0){specsHTML='<div class="modal-specs"><div class="modal-specs-title">ESPECIFICACIONES</div>'+Object.entries(item.specs).map(function(p){return '<div class="modal-spec-row"><span class="spec-key">'+p[0]+'</span><span class="spec-val">'+p[1]+'</span></div>';}).join('')+'</div>';}
  ...
  modal.innerHTML='<div class="modal-backdrop" onclick="closeModal()"></div><div class="modal-box"><button class="modal-close" onclick="closeModal()">✕</button><div class="modal-inner"><div class="modal-left"><div id="modal-gallery-wrap"></div></div><div class="modal-right"><div class="modal-meta"><span class="modal-tag tag-'+item.type+'">'+item.type.toUpperCase()+'</span><button class="modal-share-btn" onclick="shareProduct('+item.id+')">COMPARTIR ↗</button></div><div class="modal-name">'+item.name+'</div><div class="modal-price">₡'+item.price.toLocaleString()+'</div><div class="modal-stock '+getStockClass(item.stock)+'">'+getStockLabel(item.stock)+'</div><div class="modal-desc">'+item.desc+'</div>'+specsHTML+'<div class="modal-actions"><button class="btn-modal-add" onclick="addToCart('+item.id+');closeModal()"><span>AGREGAR A LA BOLSA</span></button><button class="btn-modal-wish '+(inWish?'wishlisted':'')+'" data-wish="'+item.id+'" onclick="wishFromModal('+item.id+',this)">♡</button></div>'+relatedHTML+'</div></div></div>';
...
function renderGrid(items){
  ...
  if(!document.getElementById('gallery-badge-style')){var s=document.createElement('style');s.id='gallery-badge-style';s.textContent='.product-gallery-dot{position:absolute;bottom:8px;right:8px;font-size:0.6rem;background:rgba(8,8,8,0.75);color:rgba(245,240,232,0.7);padding:3px 7px;letter-spacing:1px;pointer-events:none;}';document.head.appendChild(s);}
