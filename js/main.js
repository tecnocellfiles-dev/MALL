...
function getInventory() {
  try {
    ...
    // Si es array con productos válidos, úsalo; si está vacío o roto, usa defaults
    if (Array.isArray(parsed) && parsed.length > 0) return parsed;
    return DEFAULT_INVENTORY;
  } catch(e) {
    // Si hay corrupción, limpiar y volver a defaults
...
// ─── CURSOR ──────────────────────────────────────────────────────────────────
function initCursor() {
  // Guard: solo un loop RAF, nunca dos
...
function cardWish(id,btn){
  var added=window.toggleFavorite?window.toggleFavorite(id):false;
  btn.classList.toggle('wishlisted',added);
  showToast(added?'Guardado en favoritos':'Eliminado','info');
}
