// ═══════════════════════════════════════════════════════════
// AUTH.JS — Sistema de usuarios: clientes y administradores
// ═══════════════════════════════════════════════════════════
...
#auth-overlay {
  ...
  padding-top:72px; padding-right:6%;
  pointer-events:none; opacity:0; transition:opacity 0.25s;
}
#auth-overlay.open { opacity:1; pointer-events:all; }
.auth-panel {
  width:min(380px, 95vw);
  background:#0e0e0e; border:1px solid rgba(245,240,232,0.1);
...
.auth-panel-title {
  font-family:'Bebas Neue',sans-serif; font-size:1.1rem; letter-spacing:4px; color:var(--gold);
}
.auth-close {
...
.auth-input.admin-code-field { border-color:rgba(0,229,255,0.3); color:#00e5ff; }
...
.auth-error {
  font-size:0.72rem; color:#e05050; margin-top:10px;
  padding:8px 12px; background:rgba(224,80,80,0.08);
  border:1px solid rgba(224,80,80,0.2); display:none;
}
.auth-hint {
  font-size:0.65rem; color:rgba(245,240,232,0.25);
...
.auth-logout {
  display:block; width:calc(100% - 56px); margin:16px 28px 24px;
...
/* ORDERS */
.auth-orders { padding:0 28px 24px; }
.auth-section-title {
  font-size:0.58rem; letter-spacing:4px; text-transform:uppercase;
  color:rgba(245,240,232,0.25); margin-bottom:14px; margin-top:4px;
}
.order-card {
...
/* ADMIN BADGE EN NAV */
.nav-admin-badge {
  font-size:0.55rem; letter-spacing:2px; background:rgba(0,229,255,0.12);
...
// ─── PANEL ────────────────────────────────────────────────────────────────────
let panelOpen = false;
let currentView = 'main'; // main | orders | favs

function toggleAuthPanel() {
  panelOpen ? closeAuthPanel() : openAuthPanel();
}
...
function renderPanel() {
  ...
  if (u) {
    html = renderLoggedIn(u);
  } else {
    html = renderLoginForm();
  }

  overlay.innerHTML = `
    <div class="auth-panel">${html}</div>
  `;
  attachPanelEvents();
}
...
function renderLoggedIn(u) {
  ...
  if (currentView === 'orders') return renderOrders(u, initials);
  if (currentView === 'favs')   return renderFavs(u, initials);
  ...
  return `
    ...
    <div class="auth-menu">
      <button class="auth-menu-item" onclick="authView('orders')">
        <span class="item-icon">◳</span> Mis Pedidos <span style="margin-left:auto;font-family:'Bebas Neue';font-size:0.9rem;color:var(--gold);">${ordersCount}</span>
      </button>
      <button class="auth-menu-item" onclick="authView('favs')">
        <span class="item-icon">♡</span> Favoritos <span style="margin-left:auto;font-family:'Bebas Neue';font-size:0.9rem;color:var(--gold);">${favsCount}</span>
      </button>
      ${u.role==='admin' ? `
      <button class="auth-menu-item admin-link" onclick="window.open('admin.html','_blank');closeAuthPanel()">
        <span class="item-icon">⚙</span> Panel de Administración →
      </button>` : ''}
    </div>
...
function renderOrders(u, initials) {
  ...
  return `
    ...
    <div class="auth-orders">
      ...
      ${orders.length === 0
        ? '<div class="no-favs">Sin pedidos aún</div>'
        : orders.map(o => `
...
function renderFavs(u, initials) {
  ...
  return `
    ...
    <div class="auth-favs">
      ...
      ${favItems.length === 0
        ? '<div class="no-favs">Sin favoritos aún</div>'
        : favItems.map(item => `
...
function renderLoginForm() {
  return `
    ...
    <div class="auth-body" id="auth-form-body">
      ${loginFormHTML()}
    </div>
  `;
}
...
function registerFormHTML() {
  return `
    ...
    <div class="auth-field">
      ...
      <input class="auth-input admin-code-field" type="text" id="auth-code" placeholder="Déjalo vacío si eres cliente" style="font-size:0.8rem;letter-spacing:2px;">
    </div>
    <div class="auth-error" id="auth-error"></div>
...
// ─── INYECTAR CONTENEDORES EN EL DOM ─────────────────────────────────────────
function injectContainers() {
  ...
  if (!document.getElementById('auth-backdrop')) {
    ...
    backdrop.onclick = closeAuthPanel;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Exponer isAdmin globalmente para admin.html
